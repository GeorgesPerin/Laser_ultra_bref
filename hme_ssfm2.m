...
g2=-2*(g1^2/g0/Psatf)*real(dot(uf,(L + g1 * K).* uf));
u=ifft(ifftshift(exp(L *h/2 + (g1 * h/2 + g2/8 * h^2)*K).* uf));
uf=fftshift(fft(exp(-(del + 1i * gam)/(2 * del)*log(1-2 * del * h * abs(u).^2)).*u));
g1=g0/(1 + norm(uf)^2/Psatf);
